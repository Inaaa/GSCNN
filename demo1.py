...
parser.add_argument('--syncbn', action='store_true', default=False,  #True
...
try:
   sys.path.remove('/opt/mrtros/lib/python2.7/dist-packages')
...
class GscnnModel(object):
	...
	def __init__(self, frame):
		super(GscnnModel,self).__init__()
		self.frame = frame
		self.model = network.get_net(args,None)
	'''
		self.model =GSCNN(34)
		ck= torch.load('./checkpoints/best_cityscapes_checkpoint.pth') ## path of the checkpoint file
		print(ck.keys())
		par = ck['state_dict']  ###paremeters
		print(par)
...
def main():
	video_path ='/home/chli/cc_code/GSCNN/1.mp4'
